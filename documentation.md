...
```mermaid
graph TD
    A[runbot.sh / CRON] --> B(scraper/v2.py)
    ...
    G -->|Azure SDK| H((Azure Storage Container))
...
### 2.1 Cloud Blob Manifest (Azure Storage)
...
Array stored sequentially on Azure blob representing the core mailing group.
```json
[
  {
...
### 2.2 Local Database Constraints (`subscribers.db`)
...
2. Extracts payload logic directly utilizing an internal instance of `lib.content.get_latest_issue()`, bypassing Azure requests using internal memory exclusively.
